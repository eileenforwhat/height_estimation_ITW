...
def annotate_peak(
    ...
    np.savez(peaks_path, peak1=peaks[0], peak2=peaks[1])
...
def extract_kps_two_view(
        images_arr, 
        N=50, 
        corresp_path='corresp.npy', viz_match_path="output/viz_matches.jpg", 
        force_recompute=False
    ):
...
def show_viz_3d(peaks, priors, cameras, plane=None):
...
def height_estimation_from_ground(ground_plane, peak_coords):
    ...
    # peak_coords: (4, 1)
    num = np.abs(
        ground_plane[0] * peak_coords[0] + 
        ground_plane[1] * peak_coords[1] + 
        ground_plane[2] * peak_coords[2] + 
        ground_plane[3]
    )
...
if __name__ == '__main__':
    ...
    viz_match_path = 'data/simon/viz_matches.jpg'
    ...
    show_viz(images[0], images[1], inliers1, inliers2, F, 'inliers')
    ...
    print(f'{peak_coords1=} {peak_coords2=}')
    threeD_peak_coords = triangulate(P1, P2, peak_coords1.T, peak_coords2.T)  # (4 x 2) 3d points
    threeD_prior_coords = triangulate(P1, P2, prior_coords1.T, prior_coords2.T) # (4 x 2) 3d points
    ...
    calibrated_coords, scale_factor = calibrate([threeD_peak_coords, threeD_prior_coords, C1, C2], threeD_prior_coords, prior_height)
    calibrated_peak_coords, calibrated_prior_coords, C1, C2 = calibrated_coords
    ...
    # ground_plane = calibrate_ground_plane(
    #     P1, P2, 
    #     images[0], images[1], 
    #     'data/simon/ground_plane.npy', 
    #     scale=1, 
    #     force_recompute=True
    # )
    # show_viz_3d(calibrated_peak_coords, calibrated_prior_coords, np.array([C1, C2]), plane=ground_plane)
    # height = height_estimation_from_ground(ground_plane, threeD_peak_coords[:, 0])
